...
class FaceSwapper(object):
    PREDICTOR_PATH = "./config/shape_predictor_68_face_landmarks.dat"
    SCALE_FACTOR = 1
    FEATHER_AMOUNT = 9
    ...
    def get_face_mask(self, im, landmarks):
        ...
        im = (
            cv2.GaussianBlur(im, (self.FEATHER_AMOUNT, self.FEATHER_AMOUNT), 0) > 0
        ) * 1.0
    ...
    def read_im_and_landmarks(self, fname, use_decode=False):
        if use_decode == True:
            im = cv2.imdecode(fname, cv2.IMREAD_COLOR)
        else:
            im = cv2.imread(fname, cv2.IMREAD_COLOR)
        im = cv2.resize(
            im, (im.shape[1] * self.SCALE_FACTOR, im.shape[0] * self.SCALE_FACTOR)
        )
    ...
    def run(self, face_img):
        base_img = 'thrones/' + random.choice(os.listdir('thrones'))
        face_img = numpy.frombuffer(face_img, numpy.uint8)
        im1, landmarks1 = self.read_im_and_landmarks(base_img)
        im2, landmarks2 = self.read_im_and_landmarks(face_img, use_decode=True)

        M = self.transformation_from_points(
            landmarks1[self.ALIGN_POINTS], landmarks2[self.ALIGN_POINTS]
        )

        mask = self.get_face_mask(im2, landmarks2)
        warped_mask = self.warp_im(mask, M, im1.shape)
        combined_mask = numpy.max(
            [self.get_face_mask(im1, landmarks1), warped_mask], axis=0
        )
        ...
        filename = 'output/{}.jpg'.format(int(time()*1000))
